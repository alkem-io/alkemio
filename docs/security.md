...
* Routing to the Identity Provider endpoints is done via standard 3rd party libraries
    * For Alkemio Client.Web – Microsoft Authentication Library for JavaScript (e.g. MSAL.js)
...
* Accessing 3rd party APIs requires obtaining API specific access token.
 ...
 <p>
<img src="images/security-network.png" alt="AAlkemio Security Overview" width="600" />
</p>
...
The Access Token is passed as a Bearer token in the Authorization HTTP header (e.g. a header in the form of ‘Bearer [access_token]’). The bearer token is used in the Alkemio Server to authorize Alkemio API calls.
...
 
##	Authorization
The following roles are used within the context of the Alkemio platform:
...
* **Global Admin** - Full access for both Alkemio API and AAD.
